...
public class WiimoteDemo : MonoBehaviour {
    ...
    public Transform UnModel;
    public Transform UnModelWrist;
	...
	void Update () {
        ...
        do
        {
            ...
            if (ret > 0 && wiimote.current_ext == ExtensionController.MOTIONPLUS) {
                ...
                //Vector3 offset = new Vector3(0, -wiimote.MotionPlus.PitchSpeed, -wiimote.MotionPlus.YawSpeed) / 100;
                if(UnModel.localRotation.x > 0 && UnModel.localRotation.x < 140)
                {
                    RotX = -wiimote.MotionPlus.RollSpeed;
                } else
                {
                    RotX = 0;
                }

                if (UnModel.localRotation.y > -160 && UnModel.localRotation.y < 170)
                {
                    RotY = -wiimote.MotionPlus.YawSpeed;
                }
                else
                {
                    RotY = 0;
                }
                //Debug.Log("X axis: " + RotX + "  Y axis: " + RotY);
                Debug.Log(UnModel.localRotation);
                RollInverter = (wiimote.MotionPlus.RollSpeed) - ((wiimote.MotionPlus.RollSpeed) * 2);
                Vector3 offset = new Vector3(Mathf.Round(RollInverter),0 , (Mathf.Round(RotY))) / 100;
                wmpOffset += offset;

                //model.rot(offfset, Space.Self);
                //UnModel.Rotate   (offset);

                //UnModel.Rotate(0, (Mathf.Round((wiimote.MotionPlus.YawSpeed / 10)) / 10), (Mathf.Round((wiimote.MotionPlus.RollSpeed / 10)) / 10));
                
                UnModel.Rotate(offset);
                //UnModelWrist.Rotate(0, -wiimote.MotionPlus.RollSpeed / 100, 0);
                //UnModel.Rotate(offset);

            }
        } while (ret > 0);
        //transform.Rotate(model.rot.rotation * GetAccelVector() * 2);
        ...
        */
        //model.rot.Rotate(model.rot.rotation * GetAccelVector() * 2, Space.Self);
